...
def segregate_files():
    ...
    if directory:
        ...
        collapse_button.config(state=tk.DISABLED)
    else:
...
def segregate_selected_types():
    segregate.check_type = [image_var.get(), video_var.get(), audio_var.get(), document_var.get(
    ), programming_var.get(), application_var.get(), setup_var.get(), others_var.get()]
    segregate_files()
    # Disable the collapse button
    collapse_button.config(state=tk.DISABLED)
...
def collapse_subdirectories():
    directory = directory_var.get()
    if directory:
        segregate.collapse_directory(directory)
        result_label.config(
            text="Subdirectory files collapsed into the main directory.")
    else:
        result_label.config(text="Please select a directory.")
...
directory_entry = ttk.Entry(
    directory_frame, textvariable=directory_var, width=40)
directory_entry.grid(row=0, column=1, padx=5, pady=5)

# Browse button
browse_button = ttk.Button(
    directory_frame, text="Browse", command=browse_directory)
browse_button.grid(row=0, column=2, padx=5, pady=5)

# Segregate button
segregate_button = ttk.Button(
    root, text="Segregate Files", command=segregate_files)
segregate_button.pack(pady=5)

# Segregate by type button
segregate_by_type_button = ttk.Button(
    root, text="Segregate by Type", command=segregate_by_type)
segregate_by_type_button.pack(pady=5)

# Collapse subdirectory files button
collapse_button = ttk.Button(
    root, text="Collapse Subdirectory Files", command=collapse_subdirectories)
collapse_button.pack(pady=5)
...
image_checkbox = ttk.Checkbutton(
    check_type_frame, text="Images", variable=image_var)
image_checkbox.grid(row=0, column=0, padx=5, pady=5, sticky=tk.W)

video_var = tk.IntVar()
video_checkbox = ttk.Checkbutton(
    check_type_frame, text="Videos", variable=video_var)
video_checkbox.grid(row=0, column=1, padx=5, pady=5, sticky=tk.W)

audio_var = tk.IntVar()
audio_checkbox = ttk.Checkbutton(
    check_type_frame, text="Audios", variable=audio_var)
audio_checkbox.grid(row=1, column=0, padx=5, pady=5, sticky=tk.W)

document_var = tk.IntVar()
document_checkbox = ttk.Checkbutton(
    check_type_frame, text="Documents", variable=document_var)
document_checkbox.grid(row=1, column=1, padx=5, pady=5, sticky=tk.W)

programming_var = tk.IntVar()
programming_checkbox = ttk.Checkbutton(
    check_type_frame, text="Programming Files", variable=programming_var)
programming_checkbox.grid(row=2, column=0, padx=5, pady=5, sticky=tk.W)

application_var = tk.IntVar()
application_checkbox = ttk.Checkbutton(
    check_type_frame, text="Applications", variable=application_var)
application_checkbox.grid(row=2, column=1, padx=5, pady=5, sticky=tk.W)

setup_var = tk.IntVar()
setup_checkbox = ttk.Checkbutton(
    check_type_frame, text="Setup Files", variable=setup_var)
setup_checkbox.grid(row=3, column=0, padx=5, pady=5, sticky=tk.W)

others_var = tk.IntVar()
others_checkbox = ttk.Checkbutton(
    check_type_frame, text="Others", variable=others_var)
others_checkbox.grid(row=3, column=1, padx=5, pady=5, sticky=tk.W)

segregate_types_button = ttk.Button(
    check_type_frame, text="Segregate Selected Types", command=segregate_selected_types)
